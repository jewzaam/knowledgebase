...
| [Settings Rewrites](claude-code/settings-json-rewrites.md) | What survives a `settings.json` rewrite: top-level unknown keys yes, nested hook/rule keys no, command strings and their shell comments yes |
...
| [Skills and Plugins](codex-code/skills-and-plugins.md) | `.agents/skills` discovery scopes, initial-list truncation budget, Agent Skills frontmatter vs Claude Code's dialect, absent `!`-injection, `request_user_input` and its `default_mode_request_user_input` flag (plus fallbacks if removed) vs the unshipped `ask_user_question`, plugin-namespaced `$plugin:skill` invocation, `.codex-plugin/plugin.json` and marketplace sources, `codex exec` structured-output behavior |
